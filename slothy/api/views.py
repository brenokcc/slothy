            ...
            if len(tokens) == 1:
                ...
                if len(tokens) > 1:
                    if tokens[1] == 'dir':  # dir app
                        response.update(data=utils.dir_app(tokens[0]))
                    else:
                        model = apps.get_model(tokens[0], tokens[1])
                        if len(tokens) > 2:
                            if tokens[2] == 'dir':  # dir model
                                response.update(data=utils.dir_model(model))
                            elif tokens[2] == 'add':  # add
                                func = model.objects.add
                                data = utils.apply(model, func, {'instance': data}, request.user)
                                ...
                                data = utils.apply(model, func, data, request.user)
                                    ...
                                    if tokens[3] == 'dir':
                                        ...
                                        if len(tokens) > 4:  # add or remove
                                            ...
                                            if tokens[4] == 'add':
                                                ...
                                                data = utils.apply(model, qs.remove, {'instance': data}, request.user, relation_name=tokens[3])
